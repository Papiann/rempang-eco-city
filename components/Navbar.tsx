...
const navLinks = [
  ...
  { label: "Profil REC", href: "/profil" },
  {
    ...
    href: "/koperasi",
    items: [
      { label: "Koperasi Transmigrasi", href: "/koperasi" },
      { label: "Koperasi Merah Putih", href: "/koperasi" },
    ],
  },
    ...
    >
      <nav className="container-content flex items-center justify-between py-4 md:py-5">
        <Link href="/" className="flex items-center gap-3 text-primary-blue">
          ...
          />
          <span className="font-heading font-bold text-lg md:text-xl">Rempang Eco City</span>
        </Link>
          ...
          >
            <div className="container-content py-4 space-y-2">
              {navLinks.map((link) => {
                if ("items" in link && link.items) {
                  return (
                    <div key={link.label} className="rounded-lg border border-border-color bg-bg-light/40">
                      <button
                        type="button"
                        onClick={() => setOpenDropdown((prev) => (prev === link.label ? null : link.label))}
